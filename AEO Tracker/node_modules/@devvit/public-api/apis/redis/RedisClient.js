...
export class RedisClient {
    constructor(metadata, storage = undefined) {
    ...
    async get(key) {
        const response = await this.storage.Get({ key }, __classPrivateFieldGet(this, _RedisClient_metadata, "f"));
    ...
    async del(...keys) {
        await this.storage.Del({ keys }, __classPrivateFieldGet(this, _RedisClient_metadata, "f"));
    }
    async incrBy(key, value) {
        const response = await this.storage.IncrBy({ key, value }, __classPrivateFieldGet(this, _RedisClient_metadata, "f"));
        return response.value;
    }
    async getRange(key, start, end) {
        const response = await this.storage.GetRange({ key, start, end }, __classPrivateFieldGet(this, _RedisClient_metadata, "f"));
        return response !== null ? response.value : response;
    }
    async setRange(key, offset, value) {
        const response = await this.storage.SetRange({ key, offset, value }, __classPrivateFieldGet(this, _RedisClient_metadata, "f"));
        return response.value;
    }
    async strlen(key) {
        const response = await this.storage.Strlen({ key }, __classPrivateFieldGet(this, _RedisClient_metadata, "f"));
        return response.value;
    }
    async expire(key, seconds) {
        await this.storage.Expire({ key, seconds }, __classPrivateFieldGet(this, _RedisClient_metadata, "f"));
    }
    async expireTime(key) {
        const response = await this.storage.ExpireTime({ key }, __classPrivateFieldGet(this, _RedisClient_metadata, "f"));
        return response.value;
    }
    async zAdd(key, ...members) {
        return (await this.storage.ZAdd({ key, members }, __classPrivateFieldGet(this, _RedisClient_metadata, "f"))).value;
    }
    async zRange(key, start, stop, options) {
        // eslint-disable-next-line
        let opts = { rev: false, byLex: false, byScore: false, offset: 0, count: 1000 };
        ...
        return (await this.storage.ZRange({ key: { key: key }, start: start + '', stop: stop + '', ...opts }, __classPrivateFieldGet(this, _RedisClient_metadata, "f"))).members;
    }
    async zRem(key, members) {
        const response = await this.storage.ZRem({ key: { key }, members }, __classPrivateFieldGet(this, _RedisClient_metadata, "f"));
        return response.value;
    }
    async zRemRangeByLex(key, min, max) {
        const response = await this.storage.ZRemRangeByLex({ key: { key }, min, max }, __classPrivateFieldGet(this, _RedisClient_metadata, "f"));
        return response.value;
    }
    async zRemRangeByRank(key, start, stop) {
        const response = await this.storage.ZRemRangeByRank({ key: { key }, start, stop }, __classPrivateFieldGet(this, _RedisClient_metadata, "f"));
        return response.value;
    }
    async zRemRangeByScore(key, min, max) {
        const response = await this.storage.ZRemRangeByScore({ key: { key }, min, max }, __classPrivateFieldGet(this, _RedisClient_metadata, "f"));
        return response.value;
    }
    async zScore(key, member) {
        const response = await this.storage.ZScore({ key: { key }, member }, __classPrivateFieldGet(this, _RedisClient_metadata, "f"));
        return response.value;
    }
    async mget(keys) {
        const response = await this.storage.MGet({ keys }, __classPrivateFieldGet(this, _RedisClient_metadata, "f"));
        return response !== null ? response.values.map((value) => value || null) : response;
    }
    async mset(keyValues) {
        const kv = Object.entries(keyValues).map(([key, value]) => ({ key, value }));
        await this.storage.MSet({ kv }, __classPrivateFieldGet(this, _RedisClient_metadata, "f"));
    }
    async zCard(key) {
        const response = await this.storage.ZCard({ key }, __classPrivateFieldGet(this, _RedisClient_metadata, "f"));
        return response.value;
    }
    async type(key) {
        const response = await this.storage.Type({ key: key }, __classPrivateFieldGet(this, _RedisClient_metadata, "f"));
        return response.value;
    }
    async hget(key, field) {
        const response = await this.storage.HGet({ key, field }, __classPrivateFieldGet(this, _RedisClient_metadata, "f"));
        return response !== null ? response.value ?? undefined : response;
    }
    async hset(key, fieldValues) {
        const fv = Object.entries(fieldValues).map(([field, value]) => ({ field, value }));
        const response = await this.storage.HSet({ key, fv }, __classPrivateFieldGet(this, _RedisClient_metadata, "f"));
        return response.value;
    }
    async hgetall(key) {
        const response = await this.storage.HGetAll({ key }, __classPrivateFieldGet(this, _RedisClient_metadata, "f"));
        return response.fieldValues;
    }
    async hdel(key, fields) {
        const response = await this.storage.HDel({ key, fields }, __classPrivateFieldGet(this, _RedisClient_metadata, "f"));
        return response.value;
    }
    async hscan(key, cursor, pattern, count) {
        const request = { key, cursor, pattern, count };
        return await this.storage.HScan(request);
    }
    async hkeys(key) {
        const response = await this.storage.HKeys({ key });
