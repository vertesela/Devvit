...
var _Upload_instances, _Upload_canImportPublicAPI, _Upload_promptNameUntilNotTaken, _Upload_checkAppNameAvailability, _Upload_getAppDescription, _Upload_promptForNSFW, _Upload_syncPublishedAndLocalVersions, _Upload_syncAssets, _Upload_getAssets;
import { ActorSpec, AppCreationRequest, AppVersionCreationRequest, FullAppInfo, GetAppBySlugRequest, InstallationType, UploadNewMediaRequest, VersionVisibility, } from '@devvit/protos';
...
import { ACTOR_SRC_DIR, ACTOR_SRC_PRIMARY_NAME, ACTORS_DIR_LEGACY, ASSET_HASHING_ALGO, } from '@devvit/shared-types/constants.js';
import tinyglob from 'tiny-glob';
import { createHash } from 'crypto';
import { handleTwirpError } from '../util/twirp-error-handler.js';
import { ALLOWED_ASSET_EXTENSIONS, ASSET_DIRNAME, MAX_ASSET_FOLDER_SIZE_BYTES, MAX_ASSET_GIF_SIZE, MAX_ASSET_NON_GIF_SIZE, prettyPrintSize, } from '@devvit/shared-types/Assets.js';
import { MY_PORTAL_ENABLED } from '@devvit/dev-server/server/config.js';
import { getCaptcha } from '../util/captcha.js';
import { exec } from 'node:child_process';
class Upload extends ProjectCommand {
    constructor() {
        super(...arguments);
        _Upload_instances.add(this);
        this.appClient = createAppClient(this);
        this.appVersionClient = createAppVersionClient(this);
    }
    async run() {
        ...
        let appInfo = await this.getAppBySlug(appName);
        if (!flags.justDoIt) {
            // If we're not just doing it, check and make sure there's a chance our
            // build will succeed.
            if (!fs.existsSync(path.join(this.projectRoot, '.pnp.cjs')) &&
                !(await __classPrivateFieldGet(this, _Upload_instances, "m", _Upload_canImportPublicAPI).call(this))) {
        ...
        // we'll create a new app if
        // 1. the slug doesn't exist, or
        // 2. the existing app wasn't created by the current user
        let didVerificationBuild = false;
        if (appInfo == null || appInfo.app?.owner?.displayName !== username) {
        ...
        // Otherwise, create a new version, probably prompting for the new version number
    ...
    async createNewApp(projectConfig, copyPaste, justDoIt) {
        projectConfig.name = await __classPrivateFieldGet(this, _Upload_instances, "m", _Upload_promptNameUntilNotTaken).call(this, sluggable(projectConfig.name) ? makeSlug(projectConfig.name) : undefined);
        const description = await __classPrivateFieldGet(this, _Upload_instances, "m", _Upload_getAppDescription).call(this);
        const isNsfw = justDoIt ? false : await __classPrivateFieldGet(this, _Upload_instances, "m", _Upload_promptForNSFW).call(this);
        const categories = []; // TODO: should prompt in the future
        ...
        if (DevvitVersion.fromString(version).newerThan(new DevvitVersion(0, 0, 0))) {
            this.warn(`The version number in your devvit.yaml is larger than "0.0.0". The first published version of your app must be "0.0.0".
        ...
        try {
            ...
            if (err instanceof TwirpError && err.code === TwirpErrorCode.AlreadyExists) {
                this.error(`An app account with the name "${projectConfig.name}" already exists. Please change the "name" field of devvit.yaml and try again.`);
            }
            else {
    ...
    async getNextVersion(appInfo, bump, prompt) {
        // Sync up our local and remote versions
        const appVersion = await __classPrivateFieldGet(this, _Upload_instances, "m", _Upload_syncPublishedAndLocalVersions).call(this, appInfo, !prompt);
    ...
    async createVersion(appInfo, appVersion, bundles, visibility) {
        // Get the 'about' text
        let about = '';
        const readmePath = (await glob('*.md', { cwd: this.projectRoot })).filter((file) => file.toLowerCase() === 'readme.md');
        ...
        if (readmePath.length >= 1) {
            ...
            this.log("Couldn't find README.md, so not setting an 'about' for this app version (you can update this later)");
        }
        // Sync and upload assets
        const assetNamesToIDs = await __classPrivateFieldGet(this, _Upload_instances, "m", _Upload_syncAssets).call(this);
    ...
    async bundleActors(username, version, typecheck = true) {
        ...
        try {
            ...
            if (await dirExists(srcDirPath)) {
                 ...
                 */
                return [
                    await bundler.bundle(srcDirPath, ActorSpec.fromPartial({
                ...
                return await Promise.all(actorDirs.map((actorDir) => bundler.bundle(actorDir, ActorSpec.fromPartial({
    ...
    getLatestPublishedVersion(versions) {
...
_Upload_instances = new WeakSet(), _Upload_canImportPublicAPI = function _Upload_canImportPublicAPI() {
...
}, _Upload_promptNameUntilNotTaken = async function _Upload_promptNameUntilNotTaken(appName) {
    for (;;) {
        ...
        appName = rsp.appName;
        if (appName) {
            const isAvailableResponse = await __classPrivateFieldGet(this, _Upload_instances, "m", _Upload_checkAppNameAvailability).call(this, appName);
            ...
            if (isAvailableResponse.suggestions.length > 0) {
                this.log(`Here's some suggestions:\n  * ${isAvailableResponse.suggestions.join('\n  * ')}`);
                appName = isAvailableResponse.suggestions[0];
            }
        }
    }
}, _Upload_checkAppNameAvailability = async function _Upload_checkAppNameAvailability(name) {
...
}, _Upload_getAppDescription = async function _Upload_getAppDescription() {
    return (await readPackageJSON(this.projectRoot)).description?.substring(0, 200);
}, _Upload_promptForNSFW = async function _Upload_promptForNSFW() {
...
}, _Upload_syncPublishedAndLocalVersions = async function _Upload_syncPublishedAndLocalVersions(appInfo, justDoIt) {
    const latestStoredVersion = this.getLatestPublishedVersion(appInfo.versions);
    ...
    if (!latestStoredVersion.isEqual(devvitYamlVersion)) {
        this.warn(`The latest published version on Reddit is "${latestStoredVersion.toString()}". The version number in your local devvit.yaml is "${devvitYamlVersion.toString()}"`);
        const overwriteVersion = justDoIt ||
            (await inquirer.prompt([
                {
                    name: 'overwriteVersion',
                    type: 'confirm',
                    message: 'Allow overwriting local devvit.yaml to match versions with the latest published version on Reddit?',
                },
            ])).overwriteVersion;
        if (!overwriteVersion) {
            this.error(`Aborting. Make sure to manually set the version field of devvit.yaml to "${latestStoredVersion.toString()}" to match the latest published version already on Reddit.`);
        }
        await updateDevvitConfig(this.projectRoot, {
 ...
 */
async function _Upload_syncAssets() {
    const assetNamesToIDs = {};
    const config = await this.getProjectConfig();
    const appAssets = await __classPrivateFieldGet(this, _Upload_instances, "m", _Upload_getAssets).call(this);
    ...
    if (appAssetsTotalSize > MAX_ASSET_FOLDER_SIZE_BYTES) {
        this.error(`Your assets folder is too big - you've got ${prettyPrintSize(appAssetsTotalSize)} of assets, which is more than the ${prettyPrintSize(MAX_ASSET_FOLDER_SIZE_BYTES)} total allowed.`);
    }
    for (const asset of appAssets) {
        if (asset.filePath.endsWith('.gif') && asset.size > MAX_ASSET_GIF_SIZE) {
            this.error(`Asset ${asset.filePath} is too large - gifs can't be more than ${prettyPrintSize(MAX_ASSET_GIF_SIZE)}.`);
        }
        if (asset.size > MAX_ASSET_NON_GIF_SIZE) {
            this.error(`Asset ${asset.filePath} is too large - images can't be more than ${prettyPrintSize(MAX_ASSET_NON_GIF_SIZE)}.`);
        }
    }
    ...
    ux.action.stop(`Found ${filesNeedingNewUpload.length} new asset${filesNeedingNewUpload.length === 1 ? '' : 's'}.`);
    ...
    const uploadResults = await Promise.all(filesNeedingNewUpload.map(async (f) => {
        return [
            f,
            await this.appClient.UploadNewMedia(UploadNewMediaRequest.fromPartial({
                slug: config.name,
                size: f.size,
                hash: f.hash,
                contents: f.contents,
    ...
    return assetNamesToIDs;
}, _Upload_getAssets = async function _Upload_getAssets() {
...
Upload.description = 'Upload the app to the App Directory. Uploaded apps are only visible to you (the app owner) and can only be installed to a small test subreddit with less than 50 subscribers';
Upload.flags = {
    bump: Flags.custom({
        name: 'bump',
